...
#fitting the svm model onto the training data:
library(e1071)
classifier<-svm(survived~.,data=train,type='C-classification',kernel='linear')
summary(classifier)

#making predictions onto the test set data:
...
cv<-lapply(fold, function(x){
  ...
  test_fold<-train[x,]
  classifier<-svm(survived~.,data=training_fold,type='C-classification',kernel='linear')
