...
#fitting the naive-bayes model onto the training set data:
library(e1071)
classifier<-naiveBayes(x=train[-1],y=train$survived)
...
cv<-lapply(fold, function(x){
  ...
  test_fold<-train[x,]
  classifier<-naiveBayes(x=training_fold[-1],y=training_fold$survived)
