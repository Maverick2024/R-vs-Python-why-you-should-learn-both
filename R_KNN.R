...
#fitting the classifier onto the training set and making predictions on the test set data:
library(class)
y_pred<-knn(train=train[,-1],test[,-1],cl=train[,1],k=5)
...
cv<-lapply(fold,function(x){
    ...
    test_fold<-train[x,]
    #implementing the classifier and predicting the results:
    y_pred<-knn(train=training_fold[,-1],test=test_fold[,-1],cl=training_fold[,1],k=5)
    #confusion_matrix:
...
rm(df,df_processed,sex_group,splitIndex,test,train,accuracy,cm,cv,fold,y_pred)
